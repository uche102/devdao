...
export function DevDAODashboard() {
  ...
  const displayedMembers = contractConfigured ? memberCount : 0;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!contractConfigured) {
      toast.error("Contract not configured", {
        description:
          "Set NEXT_PUBLIC_CONTRACT_ADDRESS to enable creating proposals.",
      });
      return;
    }
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {!contractConfigured && (
          <div className="mb-6 rounded-lg border border-amber-400/25 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
            Contract not configured. Set NEXT_PUBLIC_CONTRACT_ADDRESS to connect
            to the live GenLayer contract.
          </div>
        )}
          ...
          <div className="space-y-8">
            <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <StatCard
                ...
                value={contractConfigured ? "Live" : "0 USDC"}
